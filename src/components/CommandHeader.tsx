...
export type AppDisplayMode = "WEB_DASHBOARD" | "ANDROID_MOBILE";

interface CommandHeaderProps {
  appDisplayMode: AppDisplayMode;
  onChangeAppDisplayMode: (mode: AppDisplayMode) => void;
  activeRole: string;
    ...
    <header className="sticky top-0 z-30 border-b border-black/10 dark:border-white/10 bg-white/95 dark:bg-[#090D16]/95 backdrop-blur-md transition-colors">
      <div className="mx-auto max-w-7xl px-3 sm:px-6 py-2.5">
        ...
        <div className="flex flex-col gap-2.5 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="hidden sm:inline-flex items-center gap-1 rounded-full bg-[#0061A4]/15 dark:bg-cyan-500/20 px-2.5 py-0.5 text-[11px] font-bold text-[#0061A4] dark:text-cyan-300">
                  ...
                  {appDisplayMode === "WEB_DASHBOARD" ? "داشبورد دسکتاپ" : "PWA Android"}
                </span>
              </div>
          ...
          {/* سوئیچر حالت اپلیکیشن (Web Application vs Mobile Application) و تنظیمات */}
          <div className="flex flex-wrap items-center gap-2 text-xs">
            {/* کنترلر سوئیچ برجسته: وب‌اپلیکیشن دسکتاپ در مقابل اپلیکیشن موبایل */}
            <div className="flex items-center rounded-2xl border border-black/10 dark:border-white/15 bg-[#F7F9FF] dark:bg-slate-900 p-1 shadow-inner">
              <button
                ...
                onClick={() => {
                  ...
                  onChangeAppDisplayMode("WEB_DASHBOARD");
                }}
                className={`flex items-center gap-1.5 rounded-xl px-3 py-1 font-bold transition-all ${
                  appDisplayMode === "WEB_DASHBOARD"
                    ? "bg-[#0061A4] text-white shadow-sm"
                    : "text-slate-600 dark:text-slate-400 hover:text-slate-900"
                }`}
                title="مشاهده در قالب وب‌اپلیکیشن دسکتاپ و مانیتورینگ اتاق کنترل"
              >
              ...
              <button
                ...
                onClick={() => {
                  triggerHapticFeedback(40);
                  onChangeAppDisplayMode("ANDROID_MOBILE");
                }}
                className={`flex items-center gap-1.5 rounded-xl px-3 py-1 font-bold transition-all ${
                ...
                title="مشاهده در قالب اپلیکیشن موبایل اندروید و تبلت نگهبان"
              >
            ...
            {/* در حالت اپلیکیشن موبایل: دکمه تغییر اندازه فریم گوشی هوشمند */}
            {appDisplayMode === "ANDROID_MOBILE" && (
              <button
                type="button"
                ...
                title="تغییر نما به فریم فیزیکی گوشی اندروید (Galaxy/Pixel)"
              >
                {isPhoneFrame ? (
                  <>
                    <Maximize2 className="h-3.5 w-3.5" />
                    <span className="hidden sm:inline">نمای تمام‌صفحه</span>
                  </>
                ) : (
                  <>
                    <Smartphone className="h-3.5 w-3.5" />
                    <span className="hidden sm:inline">فریم گوشی</span>
                  </>
                )}
              </button>
            )}

            {/* دکمه نصب اپلیکیشن بر روی گوشی (PWA Install) */}
            <button
              type="button"
              ...
              title="نصب اپلیکیشن بر روی گوشی اندروید یا ویندوز"
            >
              <Download className="h-3.5 w-3.5" />
              <span className="hidden md:inline">نصب اپلیکیشن</span>
            </button>
            ...
            <select
              ...
              className="rounded-2xl border border-black/10 dark:border-white/15 bg-white dark:bg-slate-900 px-2.5 py-1.5 text-xs font-bold text-[#1A1C1E] dark:text-slate-200 focus:border-[#0061A4] focus:outline-none shadow-sm"
            >
            ...
            >
              ...
              <span className="hidden sm:inline">JWT امضاشده</span>
            </button>
          </div>
        </div>

        {/* نوار تکمیلی در حالت وب‌اپلیکیشن: ناوبری افقی دسکتاپ و وضعیت شیفت ۲۴ ساعته */}
        {appDisplayMode === "WEB_DASHBOARD" && (
          <div className="mt-3 border-t border-black/10 dark:border-white/10 pt-2">
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              ...
              {/* نشانگر زمان زنده و گشت‌های دسکتاپ */}
      ...
      {/* مودال راهنمای نصب اپلیکیشن اندروید (PWA Installation) */}
      {installModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="sleek-card w-full max-w-md border border-black/15 dark:border-white/15 bg-white dark:bg-[#121826] p-6 shadow-2xl">
            <div className="flex items-center justify-between border-b border-black/10 dark:border-white/10 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm sm:text-base font-bold text-[#1A1C1E] dark:text-white">
                  نصب اپلیکیشن اندروید مرکز ابن‌سینا
                </h3>
              </div>
              <button
                type="button"
            ...
            <div className="mt-4 space-y-3 text-xs text-slate-700 dark:text-slate-300 leading-relaxed">
              <p>
                این سامانه به صورت <strong>وب‌اپلیکیشن پیشرو (Progressive Web App - PWA)</strong> استاندارد طراحی گردیده و بدون نیاز به نصب از بازار، مستقیماً روی گوشی هوشمند، تبلت سازمانی و ویندوز نصب می‌شود:
              </p>

              <div className="rounded-2xl border border-black/10 dark:border-white/10 bg-[#F7F9FF] dark:bg-slate-900 p-3.5 space-y-2">
                <div className="font-bold text-[#0061A4] dark:text-cyan-400">
                  📱 مراحل نصب در مرورگر کروم گوشی اندروید:
                </div>
                <ol className="list-decimal list-inside space-y-1 text-slate-600 dark:text-slate-400 pr-1">
                  <li>روی منوی سه‌نقطه (⋮) در بالای مرورگر کروم بزنید.</li>
                  <li>گزینه <strong>«افزودن به صفحه اصلی (Install app / Add to Home screen)»</strong> را انتخاب کنید.</li>
                  <li>آیکون اختصاصی انتظامات ابن‌سینا روی صفحه اصلی گوشی اضافه خواهد شد.</li>
                </ol>
              </div>

              <div className="rounded-2xl border border-black/10 dark:border-white/10 bg-[#F7F9FF] dark:bg-slate-900 p-3.5 space-y-2">
                <div className="font-bold text-emerald-700 dark:text-emerald-400">
                  💻 قابلیت اجرا در رایانه و اتاق مانیتورینگ:
                </div>
                <p className="text-slate-600 dark:text-slate-400">
                  در کامپیوتر یا تبلت نیز می‌توانید روی آیکون نصب در نوار آدرس مرورگر کلیک کنید تا اپلیکیشن مستقل در پنجره تمام‌صفحه اجرا شود.
                </p>
              </div>
            </div>
